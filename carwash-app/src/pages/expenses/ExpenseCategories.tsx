...
import { useExpenseCategories, useSaveExpenseCategory, useDeleteExpenseCategory } from '../../hooks/useExpenseCategories'
...
export function ExpenseCategories() {
  ...
  const [modal, setModal] = useState<{ open: boolean; editing: ExpenseCategoryItem | null }>({ open: false, editing: null })
  const [form, setForm] = useState({ name: '' })
  const [confirmDelete, setConfirmDelete] = useState<ExpenseCategoryItem | null>(null)
  ...
  function openEdit(c: ExpenseCategoryItem) {
    setForm({ name: c.name })
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Gestiona las categorías para clasificar tus gastos"
        action={
      ...
      <Card className="p-0">
        {isLoading ? (
          ...
          <EmptyState title="Sin categorías de gastos" />
        ) : (
