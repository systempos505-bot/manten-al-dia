...
export function Employees() {
  ...
  return (
    <div>
      <PageHeader
        title="Empleados"
        description="Personal, comisiones y control de turnos"
        action={
          tab === 'roster' ? (
      ...
      <div className="mb-5 inline-flex gap-1 rounded-xl bg-slate-200/70 p-1">
        <button
          ...
          className={`rounded-lg px-4 py-2 text-sm font-bold transition ${tab === 'roster' ? 'bg-white shadow text-slate-900' : 'text-slate-600'}`}
        >
          Personal
        </button>
        <button
          onClick={() => setTab('shifts')}
          className={`rounded-lg px-4 py-2 text-sm font-bold transition ${tab === 'shifts' ? 'bg-white shadow text-slate-900' : 'text-slate-600'}`}
        >
          Turnos
        </button>
      </div>
      ...
      <Modal open={modal.open} title={modal.editing ? 'Editar empleado' : 'Nuevo empleado'} onClose={() => setModal({ open: false, editing: null })}>
