        ...
        <div className="grid gap-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <Card>
              ...
              <p className="mt-1 text-2xl font-extrabold text-slate-900">{formatCurrency(data.totalSales)}</p>
            </Card>
            <Card>
              <p className="text-xs font-bold uppercase text-slate-500">Gastos totales</p>
              <p className="mt-1 text-2xl font-extrabold text-slate-900">{formatCurrency(data.totalExpenses)}</p>
            </Card>
            <Card>
              ...
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={data.salesByDay}>
                    ...
                    <Tooltip formatter={(v) => formatCurrency(Number(v))} labelFormatter={(d) => formatDate(d as string)} />
                  ...
                  {data.byEmployee.map((e) => (
                    <li key={e.name} className="flex items-center justify-between px-5 py-3 text-sm">
                      ...
                      <span className="text-right">
                        ...
                        <p className="text-xs text-slate-400">comisión {formatCurrency(e.commission)}</p>
                      </span>
                    </li>
                  ))}
                </ul>
